...
**현재 데모 버전: 0.3.3** — ESC 메뉴의 자료실(게임 내 위키), TAB 창 코드 설명, 캐릭터 선택의 기술 보기와 키보드·패드 조작. 0.3.2에서 인게임 메뉴, 캐릭터 선택의 한 줄 소개와 서포터 추천, 아스완 초반·수르트 밸런스 조정이 들어왔습니다. 0.3.1에서 macOS 빌드를 함께 배포하기 시작했고, 0.3에서 일본 해안 전선과 아베노 세이메이·스사노오 합류, 적 처치 드랍 보상이 추가되었습니다(천공 전선 0.2 포함). 버전별 변경 사항은 [패치노트](PATCH_NOTES.md)를 보세요.
