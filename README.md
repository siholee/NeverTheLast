...
**현재 데모 버전: 0.3.2** — 인게임 메뉴(설정·저장·불러오기·나가기), 캐릭터 선택 화면의 한 줄 소개와 서포터 추천, 아스완 초반·수르트 밸런스 조정. 0.3.1에서 macOS 빌드를 함께 배포하기 시작했고, 0.3에서 일본 해안 전선과 아베노 세이메이·스사노오 합류, 적 처치 드랍 보상이 추가되었습니다(천공 전선 0.2 포함). 버전별 변경 사항은 [패치노트](PATCH_NOTES.md)를 보세요.
