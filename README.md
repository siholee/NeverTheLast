...
**현재 데모 버전: 0.4.5** (Windows) / **0.4.4** (macOS) — 0.4.5에서 이아손의 궁극기가 거듭 시전 시 INT 보너스가 불어나던 문제를 고쳤습니다. 0.4.4에서 신규 서포터 **이아손**이 합류하고, 메인 메뉴에 육성 카드 열람실이 생겼으며, 같은 캐릭터를 다시 완주해도 카드가 쌓입니다. 신규 장비 9종과 계정 해금 조건 드랍, 가우디·라이트 해금 패시브도 들어왔습니다. 버전별 변경 사항은 [패치노트](PATCH_NOTES.md)를 보세요.
