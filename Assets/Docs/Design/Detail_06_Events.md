...
**세이메이 합류는 테마의 제어 안전장치다.** 봉인부 속박이 공허의 갑주를 벗길 경로를 추가하므로 츠쿠요미처럼 해금 뒤에도 이번 런에 없으면 다시 제안한다.
스사노오도 같은 규칙이다.

> **부수 효과**: 스사노오(42)·세이메이(43)는 해금 경로가 있어 **데모 즉시 사용 목록에 들지 않는다**(6.10과 같은 동작).

### 6.13 오디세이 — 돌아가지 못한 배 (`odyssey_the_long_way_home`)

| id | 방아쇠 | 조건 | 내용 |
| --- | --- | --- | --- |
| `odyssey_the_long_way_home` | 21 · 6슬롯 | — | 모래에 묻힌 배와 스무 해치 날짜. 선택지는 날짜를 더 새기거나 배를 물에 밀어 주는 둘, 모두 효과 없는 분위기 선택이다 |

**이 테마만 사건 슬롯이 5가 아니라 6이다.** 5슬롯에 세이렌 전투를 두어 돌림노래 중첩을 먼저
겪게 했기 때문이다. `RoundManager`는 사건을 `ContentSlotInRound`로만 찾으므로 슬롯 번호는
테마가 정하면 된다 — 5가 규약일 뿐 제약은 아니다.

🔸 드레이크·테세우스 합류 사건은 아직 없다. 오디세우스는 유닛이 없어 영입 사건을 두지 않았다.
