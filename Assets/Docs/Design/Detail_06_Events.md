...
| 선택지 | 액션 | 결과 |
| --- | --- | --- |
| 관중 앞에서는 모르는 척한다 | `continue` | 결승전(보스 스파르타쿠스) 복선 |
| 로마 경비대를 경고한다 | `continue` | 동일하게 결승전으로 이어짐 |

**보스전 복선형 사건.** 전력 변화 없이 서사만 담당한다.
대사에서 `portrait`와 `standing`을 직접 지정해 정체를 밝히기 전후를 연출한다
(`SHI_PORTRAIT` → `SPARTACUS_PORTRAIT`).
...
> 현재 남은 사건은 복선형 하나뿐이다. 최소한 영입형 하나는 복원하거나 새로 만들어야
...
| 공백 | 내용 |
| --- | --- |
| 🔴 사건 수 | **전체 1개.** 매 런 같은 사건만 반복된다 |
| 🔴 전력 변화 사건 없음 | 영입·아이템·패시브 부여 액션이 전부 미사용 |
| 🔸 `randomSpeakers` 미사용 | 치환 기능이 구현되어 있으나 실제 데이터에 쓰이지 않는다 |
| 🔸 보관 캐릭터 3인 | 시·케찰코아틀·츠쿠요미의 획득 사건이 필요하다 |
