...
| 적 종류 | **적다** | 한 테마 분량만 남아 있다 |
| 테마 | **매우 적다** | 🔴 **하나뿐.** 긴 여정 내내 같은 테마가 반복된다 |
| 아군 캐릭터 | **적다** | 데이터상 열이지만 |
| ↳ 그중 실제로 편성 가능한 캐릭터 | **적다** | 여섯. 기본 5인 편성은 가능하지만 조합 선택지는 적다 |
| ↳ 그중 여정을 시작할 수 있는 캐릭터 | **적다** | 수르트·아탈란테·오리온·테세우스 네 명 |
| 이야기 장면 | **매우 적다** | 🔴 하나뿐이며, 전력이 바뀌지 않는다 |
| 장비 | **매우 적다** | 🔴 중간 등급이 통째로 비어 보상이 빈약하다 |
| 재화 종류 | 충분하다 | 🔴 다만 **쓸 곳이 전혀 없다** |

> 전용 그림이 없던 적은 모두 걷어냈다. 남은 아홉은 전원 제 얼굴을 가지고 있다.
> **볼륨은 줄었지만 완성도는 올라간 상태다.** 여기서부터 다시 늘린다.

## 10. 현재 가장 큰 문제 세 가지

메인 기획서에 적을 만큼 구조적인 문제만 남긴다.

### 문제 1 — 키울 수 있는 캐릭터가 거의 없다

육성 모드는 "누구를 키울까"에서 출발하는 게임이며, 현재 스타터 네 명 모두 레벨 해금 패시브가 구현되어 있다.
선택지는 늘었지만 역할군과 속성 조합은 아직 좁으므로 다음 캐릭터도 스타터 여부를 함께 검토한다.

### 문제 2 — 여정이 한 가지 풍경뿐이다

테마가 하나라 열 번째 스테이지마다 같은 보스를 만난다.
새 테마를 붙이려면 **적의 그림이 먼저 있어야 한다.**

### 문제 3 — 벌이가 의미를 갖지 못한다

전투로 재화를 벌지만 **쓸 곳이 하나도 없다.**
유일한 소비처였던 이야기 선택마저 테마 정리와 함께 사라졌다.

## 11. 개발 방향

| 순위 | 목표 | 하는 일 |
| --- | --- | --- |
| **P0** | 키울 대상을 만든다 | 잠긴 능력 채우기 · 편성 가능 캐릭터 확충 · 동료를 얻는 이야기 복원 |
| **P1** | 루프를 닫는다 | 중간 등급 장비 추가 · 재화 소비처 도입 · 원소 규칙 정의 |
| **P2** | 여정을 넓힌다 | 새 테마(그림이 먼저) · 새 이야기 분기 · 체크포인트 보스 재설계 |
