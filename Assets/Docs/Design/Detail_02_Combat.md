...
| 유효 좌표 판정 | `x != 0 && 범위 내` | `IsValidCell()` |

- 셀 이름 규칙: `Cell_x_y`
- 실제 출전 인원은 파티 상한(5명)에 종속된다 — 셀 8칸을 다 채울 수 없다
- 대기석 유닛은 전투 판정과 UI(체력/자원 바) 모두에서 제외된다
- 유닛 사망 시 셀은 **2초 예약 대기** 후 재사용 가능해진다 (즉시 재소환 방지)
...
```
기본피해   = 스킬 위력 × 시전자 주스탯 × 0.2        // UnitStats.SkillPowerScale
유효방어력 = max(0, 방어력 − 관통) × 방어무시배율
방어배율   = 기준값 / (기준값 + 유효방어력)          // 방어력 ≥ 0
           = 2 − 기준값 / (기준값 − 유효방어력)      // 방어력 < 0
최종피해   = max(1, round(기본피해 × 치명타 × 주는피해보정 × 방어배율 × 받는피해보정))
```
...
| **방어력** | **STR × 1** (`UnitStats.DefensePerStrPoint`) |
...
**현재 용도** — 코드/패시브의 **조건 판정 트리거**

🔸 **미구현** — 원소 간 상성표, 반응(증폭·확산·결정 등), 원소 저항이 모두 없다.
토큰 8종이 원소와 1:1 대응하지만 연결 로직도 없다.
