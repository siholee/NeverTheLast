...
| 27 | 씨앗 자폭 피해 규모 | 🔴 P0 | 60레벨부터 후열 즉사. 테마 편성 전 결정 필요 |
...
수르트의 `황혼`처럼 '공격한 대상마다' 세는 코드는 소환수까지 세며,
세이의 `창세` 같은 '아군 전체'에도 소환수가 포함된다. 의도된 방향이지만 밸런스 확인이 필요하다.

## 27. 씨앗 자폭 피해 규모

파멸·종말·혹한의 씨앗은 쓰러질 때 적 진영 전체에 **자기 최대 체력의 20%(대폭발 40%)** 를
고정 피해로 준다. 기획 원안 그대로 구현했지만 스케일이 성립하지 않는다.

| 축 | 문제 |
| --- | --- |
| 기준값 | 씨앗의 **최대 체력**에 비례한다. 씨앗은 CON이 주스탯(×1.2)이고 성장이 `+3~5/레벨`이다 |
| 관통 | 고정 피해라 방어력 감쇠를 받지 않는다 |
| 범위 | 적 전체가 대상이고, 공허의 씨앗은 **일반 등급**이라 여러 마리가 나온다 |

대폭발은 `CON 40 +5`인 **종말의 씨앗**이 60레벨에 얻는다. 그 시점 피해는 **16,080**인데,
CON이 주·부 스탯이 아닌 아군의 최대 체력은 약 **7,300**이다 — 두 배가 넘는다.
40레벨(11,280 vs 5,300)에서 이미 뒤집혀 있고, 60레벨이면 CON을 주스탯으로 쓰는 전열도 위험하다.
공허의 씨앗은 일반 등급이라 여러 마리가 나오므로 광역기로 한꺼번에 잡으면 자폭이 동시에 겹친다.

| 후보 | 내용 |
| --- | --- |
| **A** | 비율을 낮춘다 (예: 10% / 20%) |
| **B** | 최대 체력 비례 대신 **고정 위력 + 시전자 스탯 비례**로 바꾼다 — 다른 코드와 같은 축이 된다 |
| **C** | 피해 상한을 대상 최대 체력의 일정 비율로 묶는다 |

테마에 편성하기 전에 결정한다. 지금은 `themeId: 0`이라 실제로 등장하지 않는다.
...
| 아트 | `GAUDI_PORTRAIT` / `GAUDI_STANDING` 키만 예약, 원본 미제작 |
