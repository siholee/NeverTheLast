...
고유 패시브 원본(`UniquePassiveCode`)은 후보에 직접 들어가지 않는다. `<전수 불가>`가 아닌 원본은 유닛 초기화 시 지정된 `TransferVersionCodeId`의 열화 일반 패시브를 `ownedPassiveCodes`에 추가하며, 현재 매핑은 아탈란테 `2→230`, 오리온 `80→231`, 아스클레피오스 `91→232`, 아마테라스 `92→233`이다. 매핑이 0인 고유 P는 전수 불가다.
