...
| 버튼 | 저장 | 도착 |
| --- | --- | --- |
| 저장 후 나가기 | 저장한다 (`CanSaveNow`일 때만 켜짐) | 메인 메뉴 |
| 나가기 | 하지 않는다 | 메인 메뉴 |
| 바탕화면으로 나가기 | 하지 않는다 | 게임 종료 |

셋 다 확인 창을 거친다. `바탕화면으로 나가기`가 몰래 저장하지 않는 것은 의도다 — "그냥 끄기"를 고른
사람의 저장본을 덮어쓰면 안 된다. 저장이 필요하면 `저장` 또는 `저장 후 나가기`를 먼저 쓴다.
...
| 인게임 메뉴 `저장` · `저장 후 나가기` | `SaveCurrentRun()` — `CanSaveNow`일 때만(아니면 버튼이 꺼진다) |
...
인게임 메뉴의 `불러오기`는 `GameStartIntent.Continue`로 Game 씬을 다시 연다. `나가기`와 `바탕화면으로 나가기`는
