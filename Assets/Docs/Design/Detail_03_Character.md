...
```
raw = base
    + incrementLvl     × 유효 성장레벨     // 아군 = Level − 1, 적은 Lv.42부터 가속
...
Lv.1~41의 성장 공식은 아군·적이 동일하다: `base + incrementLvl × (Level − 1)`.
적은 훈련 강화가 없으므로 Lv.41을 넘긴 뒤 얻는 성장분에 **×1.8**을 적용한다.

```text
아군 유효 성장레벨 = Level − 1
적 유효 성장레벨   = min(Level − 1, 40) + max(Level − 41, 0) × 1.8
```

기본 스탯, 장비·상태 보너스, 강화 횟수에는 이 배율을 적용하지 않는다. 따라서 1~40스테이지의
입문 곡선은 유지되고, 41스테이지 이후에만 아군의 누적 훈련·보상을 따라잡는다.
...
| 획득처 | 지급량 | 대상 |
| --- | --- | --- |
| 적 처치 | `4 + 1 × Stage` | 필드의 파티 전원 |
| 스테이지 클리어 | `25 + 9 × Stage` | 필드의 파티 전원 |
| 육성 페이즈 | `90 + 11 × Stage` | **메인 + 그 훈련에 서포트 카드로 앉은 서포트** |

**무게는 훈련 쪽에 실려 있다.** 전투 EXP는 다섯이 함께 받고, 훈련 EXP는 참여한 서포트만 받으므로
서포트는 메인의 약 80% 레벨에 선다. 참여 인원은 훈련을 적용하기 전에 읽는다 —
`TrainingManager.ApplyTraining`이 턴을 넘기며 배치를 무효화하기 때문이다.

> 예전에는 세 갈래 모두 파티 전원이 받아 서포트가 메인과 같은 레벨로 자랐다(문서에는 "육성은 메인만"으로
> 남아 있었으나 코드는 전원 지급이었다). 메인의 곡선은 그대로 두고 배분만 옮겼다.
...
한 슬롯 평균 적 3.8기 처치 + 클리어(5슬롯 사건 제외) + 스테이지당 훈련 0.75회(휴식 리듬)를 가정한 값이다.
서포트 참여율은 특기 훈련에 앉을 확률(카드 없음 45%)과 특기 밖으로 흩어질 확률을 감안해 5~45%로 잡았다.

| 스테이지 | 메인 Lv | 서포트 Lv (참여 5% / 25% / 45%) | 적 Lv |
| --- | --- | --- | --- |
| 10 | 10 | 7 / 7 / 8 | 10 |
| 25 | 23 | 16 / 18 / 19 | 25 |
| 50 | 44 | 33 / 35 / 38 | 50 |
...
사탕 보상(`rare_candy_t3` / `t4`)과 EXP 배율 코드(선두주자·음유시인)는 넣지 않은 값이다.
...
>
> 🔸 지금은 아군 전원이 **100으로 동일**하다. 적은 천공·해안 전선의 일부 병종과 보스가 120~150을 쓴다.
...
여섯 가지다. `ActionScheduler.ActionKind`가 그대로 이 목록이다.

| 행동 | 표기 | 턴 | AV | 설명 |
| --- | --- | :-: | :-: | --- |
| **일반행동** | `N` | 쓴다 | 리셋 | DEX가 만드는 행동치가 주기를 정한다. 쿨다운 없음 |
| **대체행동** | `N+` | 쓴다 | 리셋 | 조건이 맞으면 **일반행동 자리를 대신** 쓴다 |
| **추가행동** | `Ex` | 안 쓴다 | 유지 | 반격·연계타. 패시브가 부른 것은 같은 턴에서 **먼저** 나간다 |
| **협동행동** | `Co` | 안 쓴다 | 유지 | 짝의 행동에 끼어들어 함께 친다. §4.5-D |
| **특수행동** | `SP` | 안 쓴다 | 유지 | **인드라의 궁극기만이** 여는 행동. §4.5-C |
...
**시구르드·브륀힐드의 맹세(284·285)만이 쓴다.** 짝의 행동에 끼어들어 함께 치는 자리다.

| 항목 | 값 |
| --- | --- |
| enum | `ActionKind.Coordinated` (우선도 **3**) |
| 예약 | `ActionScheduler.EnqueueCoordinated` |
| 이벤트 | **`OnCoordinatedActivates`** — 추가행동 카운터에는 잡히지 않는다 |
| 피해 태그 | `DamageTag.CoordinatedAttack` (20008) |

**추가행동과 판정을 나눈 이유는 트리거다.** `OnAdditionalActivates`를 듣는 코드가 이미 여럿인데
(니콜의 행동 카운터, 바스테트의 스택 등), 협공이 그것들을 전부 켜면 듀오 하나 때문에
다른 코드의 균형이 흔들린다. 특수행동이 성질은 추가행동과 같으면서 판정을 따로 가진 것과 같은 이유다.

**우선도 3**은 자기를 부른 일반행동 **바로 뒤에** 붙기 위한 값이다. 추가행동(2)보다 뒤에 둬야
기존 추가행동이 끼어들 자리를 잃지 않고, 궁극기(4)보다 앞이라 협공이 먼저 떨어진다.

> **협동행동은 협동행동을 부르지 않는다.** 공명이 협동행동에서는 쌓이지 않으므로
> 시구르드 → 브륀힐드 → 시구르드로 도는 핑퐁 루프가 생기지 않는다.
> 확산이 뿌린 부착이 다시 반응하지 않는 것과 같은 차단이다.

### 4.5-C 특수행동 — 인드라가 여는 문

로카팔라 전용 행동이다. 붕괴: 스타레일의 **환락(아하)** 매커니즘을 본떴다.

| 규칙 | 내용 |
| --- | --- |
| **여는 사람** | **인드라의 궁극기 `신들의 왕` 하나뿐이다.** 다른 어떤 수단으로도 열리지 않는다 |
| 받는 사람 | 필드에 선 `Lokapala` 태그 아군 중 **SP 코드를 가진** 유닛 전원 |
| 턴 | 쓰지 않는다. AV도 리셋되지 않는다 |
| 판정 | `OnSpecialActivates`. **추가행동과 별개**라 바스테트 `야수의 시선`에는 잡히지 않는다 |
| 순위 | 우선 추가행동 바로 다음. 열린 순간 전원이 함께 나가야 하므로 추가행동보다 앞이다 |

**환락과 다른 점이 이 설계의 전부다.** 환락은 여러 경로로 자주 열리지만 이쪽은 문이 하나뿐이라,
"로카팔라를 몇 명 세웠는가"가 곧 인드라 궁극기 한 번의 값이 된다.
야마의 SP가 *함께 나선 인원수*만큼 튕기는 것도 같은 이유다.

**인드라가 미해금인 것은 의도된 설계다.** 특수행동처럼 기본 규칙을 벗어나는 추가 룰을 가진
캐릭터는 초반 유저에게 바로 쥐여 주지 않는다. 나머지 로카팔라는 SP 없이도 각자 제 몫을 하므로,
인드라를 얻기 전까지는 그냥 좋은 서포터 일곱으로 쓰면 된다 — SP는 그 위에 얹히는 보너스다.
(해금 경로는 아직 미정이다 🔴)

모든 예약은 `Combat.SpecialAction.OpenGate`를 지난다 — 다른 코드가 `EnqueueSpecial`을 직접 부르면
이 규칙이 조용히 깨지므로 여는 경로를 한 곳으로 묶어 두었다.

| 보유자 | SP | 내용 |
| --- | --- | --- |
| **바유** | 선봉의 바람 | **가장 먼저 나선다.** 아군 전체에게 이번 개방이 끝날 때까지 올스탯 +10% |
| 찬드라 | 소마의 잔 | 아군 전체 `CON×1.5` 방어막 + `INT×0.5` 마나 |
| 야마 | 죽음의 순회 | `DEX×1.2`를 **함께 나선 인원수**만큼 튕김. 각 30% 번개 |
| 아그니 | 겁화 | 적 전체 `INT×0.8` + 특수 취약 +50%(3턴). 불 보유 시 화상 3턴 |
| 인드라 | 천벌 | 옛 궁극기. `INT×[레벨]%`를 20%p씩 깎으며 튕김 |
| 쿠베라 | 골드 러쉬 | 적 전체 `STR×0.6 + CON×0.6` + 물리 취약 +50%(3턴), 골드 +5% 중첩 |
| 바루나 | 정화의 밀물 | 아군 전체 `CON×1.5` 치유 + 해로운 효과 전부 제거 |
| 수리야 | 일천 | **중첩을 태우지 않고** 최대 중첩분(10발)의 라비 |

**순서는 `SpecialCode.OpenOrder`가 정한다.** 작을수록 먼저다 — 바유만 `-1`이고 나머지는 `0`이라,
바유의 올스탯 버프가 나머지 전원의 행동에 얹힌다.

> 버프의 수명은 턴이 아니라 **이번 개방**이다. 특수행동이 턴을 쓰지 않으므로 턴으로 적으면
> 개방이 끝난 뒤에도 남는다. 마지막 한 명의 행동이 **실제로 끝난** 시점에 걷힌다
> (`SpecialAction.NotifyResolved`).
...
| **Support** | 서포터 카드로만 | ✕ | 게임 초기부터 편성 가능하지만 무한 모드에는 나오지 않는다. `unlocksAsStarterOnClear`가 붙은 넷(라이트·니콜·프레이아·마리)만 **서포트로 완주시키면** 메인 후보로 열린다 — [Detail_08 §31.2](Detail_08_Confirmed_Characters.md) |
| **Locked** | 불가 | (해금 후) | **영입 사건을 만나면**(거절해도) 영구 해금되어 이후 Starter처럼 쓴다. 해금은 사건이 결판날 때 `GameManager.GrantRecruitUnlock`이 준다 |

> **해금 경로는 셋뿐이다** — 영입 사건 · 계정 첫 완주(라부아지에) · 서포트 완주(라이트·니콜·프레이아·마리).
...
| **Support** | 21 | 세이 · 가우디 · 라이트 · 니콜 · 잔 · 위고 · 오리온 · 피그말리온 · 찬드라 · 아그니 · 바유 · 프레이아 · 스카디 · 쿠베라 · 바루나 · 오르페우스 · 아그리파 · 이아손 · 토트 · 이시스 · 마리 |
| **Locked** | 20 | 시 · 라부아지에 · 테세우스 · 아스클레피아 · 이카리아 · 아마테라스 · 츠쿠요미 · 스사노오 · 세이메이 · 야마 · 인드라 · 수리야 · 로키 · 시구르드 · 브륀힐드 · 카이사르 · 호루스 · 아누비스 · 세트 · 케찰코아틀 |

Support 21명 중 라이트·니콜·프레이아·마리는 서포트 완주 후 메인 후보로도 열린다.
Locked의 실제 해금 경로와 데모 임시 개방 여부는 [Detail_08 §31.1](Detail_08_Confirmed_Characters.md)을 따른다.
...
| `Swift` | 신속 | 추가행동을 하거나 추가행동 지원에 특화 |
...
| `Sturdy` | 견고 | 매우 높은 생존력 |
| `Summon` | 소환 | 소환수 관련 능력 보유 |
| `Indomitable` | 불굴 | 자신의 체력을 자원으로 소모하거나 그와 연관됨 |
| `Domination` | 지배 | 지속피해 또는 행동 방해 상태를 부여함 |
...
- id: 5
  ...
  unlocksAsStarterOnClear: false # 선택. 서포트로 완주하면 자신을 스타팅으로 해금
  unlocksUnitIdsOnClear: []      # 선택. 서포트로 완주하면 연결된 다른 메인을 해금
...
**총 48명.** 기초 스탯·레벨당 성장·트레이닝 보너스는 여기서 중복해 적지 않고
...
| 10 | 셰익스피어 | Starter | Geo | LUK / DEX·INT | 소환수 공격 지원형 서포터 |
| 22 | 오리온 | Support | Geo | STR / CON | 초기 서포트 카드 · 힘사수 반격 딜탱 |
| 23 | 테세우스 | 🔴 Locked | Hydro | DEX / CON | 범용 전열 딜러 |
| 80 | 수르트 | Starter | Pyro | STR / CON | 체력을 태워 때리는 전열 딜러 |
| 64 | 바유 | Starter | Anemo | CON / DEX | 정화형 방어 서포터 |
| 121 | 아누비스 | 🔴 Locked | Geo | CON / STR | 전열 탱커 · 사령 특효 |
...
| 122 | 바스테트 | Starter | Geo | LUK / DEX | 반격 탱커에 반응하는 추가행동 메인 딜러 |
...
| 103 | 이아손 | Support | Hydro | LUK / INT·STR | 방출 궁극기 반응형 공격 서포터 |
| 124 | 토트 | Support | Hydro | INT / CON | 강인도 서포트 #2 |
| 125 | 이시스 | Support | Geo | CON / INT | 범용 바위 부여 서브딜러 |
| 160 | 마리 | Support | Cryo | LUK / DEX·STR | 활·화살 치명타 특화 서포터 |
| 2 | 시 | 🔴 Locked | Anemo | DEX / LUK | 연타 암살자 |
| 20 | 피그말리온 | Support | Pyro | CON / STR·INT | 지속피해 반격 전열 탱커 |
| 24 | 아스클레피아 | 🔴 Locked | Electro | LUK / STR·CON | 정밀·치유 하이브리드 |
| 40 | 아마테라스 | 🔴 Locked | Pyro | DEX / STR·LUK | 후열 일반행동 사수 |
| 140 | 케찰코아틀 | 🔴 Locked | Dendro | INT / LUK | 풀 파티 서브 딜러·지원 |
| 41 | 츠쿠요미 | 🔴 Locked | Electro | INT / LUK | 지속피해·디버프 서포터 |
...
| 101 | 옥타비아 | Starter | Anemo | INT / LUK | 강력한 궁극기 기반 클래식 딜러 |
...
🔴 표시는 **초기 잠금 캐릭터**다. 영입 사건·완주·런 종료 등 각자의 해금 조건을 따른다.

### 7.1 획득 경로가 없는 캐릭터

테세우스와 아누비스를 포함한 일부 Locked는 아직 획득 경로가 없다.
경로가 없는 Locked는 데모에서 영구히 막히지 않도록
`CharacterSelectionManager.HasNoUnlockPath`가 임시로 편성 자격을 연다. 라부아지에는 첫 육성 완주,
야마는 모드·승패와 무관한 런 1회 종료, 나머지 일부는 영입 사건으로 해금된다.

| 막고 있는 것 | 상태 |
| --- | --- |
| 영입 사건 | 일부 Locked에 구현됨 — [Detail_06 §6](Detail_06_Events.md) |
| 해금 영구 저장 | `SaveSystem.AddStarterUnlock`으로 구현됨 |
| 사건 액션 | `grantUnitId`로 합류·영구 해금을 함께 처리 |
...
유닛마다 **고유 패시브 1개**를 가지며, 해금 패시브는 4~8개가 있다.
...
| 8 | 세이 · 오리온 |
| 7 | 아탈란테 · 찬드라 · 바스테트 |
| 6 | 시 · 아스클레피아 · 아마테라스 · 케찰코아틀 · 수르트 · 프레이아 · 야마 · 인드라 · 쿠베라 · 아그리파 · 카이사르 · 아누비스 · 라이트 |
...
| 구간 | 소속 | 인원 | 사용 중 | 캐릭터 |
| --- | --- | ---: | --- | --- |
| 1~19 | 아카샤 (드레이크·엘리자베스 포함) | 7명 | 1~5, 9~10 | 세이 · 시 · 사바흐 · 가우디 · 라이트 · 위고 · 셰익스피어 |
...
| 180~199 | 화령 | 1명 | 180 | 우치 |

세이·시·가우디·라이트·위고·셰익스피어는 세계관의 중심축인 아카샤 블록에 둔다.
우치는 기존 어느 소속에도 속하지 않아 **화령** 블록을 새로 열었다.

> 🔸 **화령이 마지막 블록이다.** 고유 패시브는 `유닛 ID + 200`이고 그 구간이 399에서 끝나므로,
> 200~ 블록을 여는 순간 고유 패시브 번호가 장비 전용(400~499)과 부딪힌다.
> 다음 진영을 열 때는 고유 패시브 구간부터 함께 넓혀야 한다 — [Detail_11 §1.5](Detail_11_Code_Catalog.md).
...
퍼지는 오라 계열(원소술사 76 · 에테르 1591 ↔ 대해의 판결 266 등)은 매 순간 높은 쪽을 골라야 하므로
...
> 서포터 카드가 넘겨줄 수 있는 것은 해금 패시브뿐이다.
> 옛 전수본 230·231·233·232는 삭제했고, 232의 효과는 전수 가능한 독립 패시브 `주문 가속`(55)으로 재편입되었다. 224는 아스클레피아의 신규 고유 패시브 `생명의 잔`이다.
