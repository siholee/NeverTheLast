...
> **역사 문서.** 케찰코아틀의 현재 확정 사양은 `Design/Detail_08_Confirmed_Characters.md`를 따른다.
> 아래 요리스틀리 기반 전투 사양은 2026-08-11 재설계로 폐기되었으며, 아즈텍 테마 구현 기록만 보존한다.
...
|---|---:|---|---:|
| 전열 방어 | 1010 | 재규어 전사 | 7 |
| 원거리 공격 | 1011 | 독수리 전사 | 8 |
| 지속 피해 | 1012 | 방울뱀 사제 | 10 |
| 아군 강화 | 1013 | 벌새 사제 | 10 |
| 광역 공격 | 1014 | 올빼미 주술사 | 10 |
| 다단 공격 | 1015 | 코요테 척후병 | 8 |
| 중간 보스 | 2003 | 틀랄록의 대사제 | 10 |
| 테마 보스 | 3002 | 불완전 현현 테스카틀리포카 | 11 |
| 시련 이벤트 보스 | 3201 | 케찰코아틀 | - |
| 적대 이벤트 보스 | 3202 | 격노한 케찰코아틀 | - |

아즈텍 테마에는 라운드 내 1, 2, 3, 4, 6, 7, 9번 위치의 전용 가중치 패턴이 등록되어 있다. 각 패턴은 병종별 역할이 유지되도록 적 ID를 직접 지정한다.

## 2. 5대 스탯

### STR

- 효과: 방어력과 장비 보유 중량 한도 증가
- 공식: 1차/2차/3차 중량 `STR / ceil(STR×1.5) / STR×2`
- 현재 장비 중량은 장착 장비와 개인 인벤토리 장비의 `weight` 합계다.
- 1차 초과는 DEX −50%, 2차 초과는 올스탯 −50%, 3차 초과는 장비 정리 외 진행 행동을 잠근다. 아이템 획득 자체는 유지된다.

### DEX

- 효과: 일반 공격 속도 증가
- 공식: `ActionSpeed = 1 + max(0, 최종 DEX) * 0.01` (독립 공격속도 스탯 없음)
...
일반 T1 장비는 ID 4101~4114를 사용한다. 전체 목록은 [Design/Detail_11_Equipment_Catalog.md](Design/Detail_11_Equipment_Catalog.md)를 따른다. 사건 전용 마카나는 ID 4010, 중량 3을 유지한다.
