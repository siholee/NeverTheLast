...
namespace Codes.Passive
{
    ...
    /// <summary>
    /// 주문 보호막(33) — 4턴마다 다음 공격 1회를 통째로 무효화하는 INT 50% 보호막을 두른다.
    /// 보호막이 대기 중인 동안에는 주기를 세지 않으므로, 맞아서 소모한 뒤부터 다시 4턴을 센다.
    /// </summary>
    public sealed class TsukuyomiSpellShield : PeriodicTurnPassive
    {
        private const int IntervalTurnCount = 4;

        private bool _armed;
        private int _grantedShield;
        private Action<EventContext> _damageHandler;

        public TsukuyomiSpellShield(PassiveCodeContext context) : base(context, IntervalTurnCount)
        {
            CodeName = "주문 보호막";
        }
        ...
        /// <summary>보호막이 대기 중이면 주기를 멈춘다 — 쌓아 둘 수 있는 코드가 아니다.</summary>
        protected override bool CanTick() => !_armed && base.CanTick();
    ...
    /// <summary>변덕쟁이(34) — 자기 턴마다 서로 다른 두 기본 스탯에 +10%/-10%를 다시 배정한다.</summary>
    public sealed class TsukuyomiFickle : PeriodicTurnPassive
    {
        private const string StatusKey = "tsukuyomi_fickle";

        public TsukuyomiFickle(PassiveCodeContext context) : base(context, 1)
        {
            CodeName = "변덕쟁이";
        }
        ...
        protected override void OnPeriodElapsed()
        {
            ...
            var status = BuffStatus.Create(
                ...
                new PrimaryStatMultiplierEffect(1.1f, (BaseEnums.PrimaryStat)stats.GetValue(raisedIndex)),
                description: "무작위 기본 스탯 하나가 10% 증가하고 다른 하나가 10% 감소합니다.");
            status.AddEffect(new PrimaryStatMultiplierEffect(0.9f, (BaseEnums.PrimaryStat)stats.GetValue(loweredIndex)));
    ...
    public sealed class TsukuyomiWidenWound : PassiveCode
    {
        ...
        private void OnDamageDealt(DamageResolvedContext context)
        {
            ...
            HealingReductionStatus.Apply(target, Caster, 3, CodeName);
        }
        public override void StopCode()
        {
    ...
    public sealed class TsukuyomiCycle : PassiveCode
    {
        ...
        private void OnAnyUnitDied(Unit dead, Unit attacker)
        {
            ...
            var status = BuffStatus.Create(
                ...
                Caster, target, new OutgoingDamageMultiplierEffect(1.25f), stackPolicy: BaseEnums.StatusStackPolicy.Stack,
                description: "가하는 피해가 25% 증가하고 받는 피해가 25% 감소합니다.");
            status.AddEffect(new ReceivingDamageMultiplierEffect(0.75f));
            target.AddStatus(status);
        }
    }
